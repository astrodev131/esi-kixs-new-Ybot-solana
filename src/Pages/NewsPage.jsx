      ...
      <div className="p-4">
        ...
        <div className="p-8 relative w-full flex-wrap flex">
          {NewsImages.map((value, index) => (
            <a href={`./news/${value}`} className="w-1/4 flex flex-col gap-2 p-2" key={index}>
              <img src={`./images/news/${value}.png`} alt={value} className="w-full" />
              <div className="text-center text-[#7a7d85] text-xs">{value}</div>
            </a>
          ))}
        </div>
      </div>
